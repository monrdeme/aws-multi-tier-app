# main.tf - Security Hub, GuardDuty, CloudTrail module
...
resource "aws_guardduty_detector" "main" {
  ...
  tags = {
    Name        = "${var.project_name}-${var.env}-guardduty-detector"
...
#   bucket = "${var.project_name}-${var.env}-guardduty-findings-${data.aws_caller_identity.current.account_id}"
#   force_destroy = true # Only for dev/testing, ensures bucket can be deleted even if it contains objects
#   tags = {
#     Name        = "${var.project_name}-${var.env}-guardduty-findings-bucket"
#     Project     = var.project_name
#     Environment = var.env
#     Service     = "GuardDuty"
#   }
...
resource "aws_s3_bucket" "cloudtrail_logs" {
  bucket        = "${var.project_name}-${var.env}-cloudtrail-logs-${data.aws_caller_identity.current.account_id}"
  force_destroy = true # Only for dev/testing, allows deleting bucket with objects

  tags = {
    Name        = "${var.project_name}-${var.env}-cloudtrail-logs-bucket"
    Project     = var.project_name
    Environment = var.env
    Service     = "CloudTrail"
  }
