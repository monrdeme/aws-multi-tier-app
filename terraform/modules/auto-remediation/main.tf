...
resource "aws_iam_policy" "remediation_policy" {
  name        = "${var.project_name}-${var.env}-remediation-policy"
...
resource "aws_lambda_function" "auto_remediation" {
  ...
  function_name = "${var.project_name}-${var.env}-auto-remediation-lambda"
  ...
  tags = {
    Name        = "${var.project_name}-${var.env}-auto-remediation-lambda"
    Project     = var.project_name
    Environment = var.env
    Service     = "AutoRemediation"
  }
...
resource "aws_cloudwatch_event_rule" "ssh_remediation_rule" {
  name        = "${var.project_name}-${var.env}-ssh-remediation-rule"
...
resource "aws_cloudwatch_event_rule" "unapproved_ami_remediation_rule" {
  name        = "${var.project_name}-${var.env}-unapproved-ami-remediation-rule"
